...
class OptimalWingState:
    ...
    def capture_complete_state(self, 
        ...
        try:
            # Use the original aerodynamic results (not the JSON-serialized version)
            # to avoid type conversion issues in constraint functions
            if full_aero_results is not None:
                constraint_penalty, hard_violation = internal_run_constraint_suite(
                    cst_parameters=best_cst_dict,
                    aero_results=full_aero_results,  # Use original results, not serialized ones
                ...
                self.state_data['constraint_results'] = {
                    'error': 'No aerodynamic results available for constraint evaluation',
        ...
        for model_name, data in optimization_log.items():
            ...
            for key, value in data.items():
                if key == 'best_parameters_per_epoch':
                    ...
                    serializable_log[model_name][key] = []
                    for item in value:
                        if hasattr(item, 'tolist'):
                            serializable_log[model_name][key].append(item.tolist())
                        elif isinstance(item, (np.integer, np.floating)):
                            serializable_log[model_name][key].append(float(item))
                        else:
                            serializable_log[model_name][key].append(item)
                else:
    ...
    def save_to_files(self, base_path: str, model_name: str):
        ...
        try:
            ...
            print(f"   ❌ CST save failed: {e}")
            cst_path = None
        
        print(f"💾 Complete state saved!")
